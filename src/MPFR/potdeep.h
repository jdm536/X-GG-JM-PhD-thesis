...
#define POT_DEEP_DELTA 0.99

// Pot-Deep index search
template <typename FP_T>
inline bool index_search_PotDeep (
                FP_mat<FP_T> &mu,
                vector<FP_NR<FP_T>> &B,
                int d,
                FP_NR<FP_T> delta,
                ...
                ) 
{
    ...
    FP_NR<FP_T> sum = 0.0;
    FP_NR<FP_T> P = 1.0;
    FP_NR<FP_T> P_min = 1.0;
    FP_NR<FP_T> P_mult = 0.0;
...
// Pot-Deep reduction for ZZ_mat<mpz_t> type basis
int potdeep (
                ZZ_mat<mpz_t> &A,
                FP_NR<mpfr_t> delta,
                int d,
                ...
                )
{
    ...
    while (size_reduction_required || deep_insertion_required)
    {
        ...
        while (k<d)
        {
            ...
            // Index search
            deep_insertion_required = index_search_PotDeep (mu, B, d, delta, k, i, start_index, end_index);
        ...
        // Check the Pot-Deep_Reducedness
        compute_GSO (A, gso, mu, B, d, start_index, end_index);
...
// Pot_Deep reduction for Z_T ** type basis
                ...
                )
{
    ...
    while (sublattice_start < end_index)
    {
		...
		cout << "in switch case mpz_t..." << endl;
                prec_status = potdeep (A, delta_Pot, d, precision, precision_correction_loops_allowed, eta, beta, gso, mu, B, sublattice_start, sublattice_end);
                break;
            case TYPE_LONG_LONG_INT:
		cout << "in switch case long long int..." << endl;
                prec_status = potdeep (ppLongLongIntA, delta_Pot, d, precision, precision_correction_loops_allowed, eta, beta, gso, mu, B, sublattice_start, sublattice_end);
                break;
            case TYPE_LONG_INT:
		cout << "in switch case long int..." << endl;
                prec_status = potdeep (ppLongIntA, delta_Pot, d, precision, precision_correction_loops_allowed, eta, beta, gso, mu, B, sublattice_start, sublattice_end);
                break;
            case TYPE_INT:
		cout << "in switch case int..." << endl;
                prec_status = potdeep (ppIntA, delta_Pot, d, precision, precision_correction_loops_allowed, eta, beta, gso, mu, B, sublattice_start, sublattice_end);
                break;
            default:
                cout << "ERROR: BASIS INTEGER DATATYPE NOT SUPPORTED" << endl;
                exit(ERROR_INVALID_INTEGER_TYPE_IN_BASIS);
    ...
    {
        cout << "Preprocessing complete. Now running PotGG globally." << endl;
        switch (int_type)
        {
            case TYPE_MPZ_T:
                prec_status = potdeep (A, delta_Pot, d, precision, precision_correction_loops_allowed, eta, beta, gso, mu, B, start_index, end_index);
                break;
            case TYPE_LONG_LONG_INT:
                prec_status = potdeep (ppLongLongIntA, delta_Pot, d, precision, precision_correction_loops_allowed, eta, beta, gso, mu, B, start_index, end_index);
                break;
            case TYPE_LONG_INT:
                prec_status = potdeep (ppLongIntA, delta_Pot, d, precision, precision_correction_loops_allowed, eta, beta, gso, mu, B, start_index, end_index);
                break;
            case TYPE_INT:
                prec_status = potdeep (ppIntA, delta_Pot, d, precision, precision_correction_loops_allowed, eta, beta, gso, mu, B, start_index, end_index);
                break;
            default:
                cout << "ERROR: BASIS INTEGER DATATYPE NOT SUPPORTED" << endl;
                exit(ERROR_INVALID_INTEGER_TYPE_IN_BASIS);
